## Bagging algorithm using entropy (Classifier+Regressor)
...
def bagging(train_df, test_df, ml_task, max_depth, sample_size, n_trees):
    
    trees = []
    for i in range(n_trees):
        bootstrap_sample = bootstrap_sampling(train_df, sample_size)
        tree = decision_tree_algorithm(train_df, ml_task=ml_task, max_depth=max_depth)
        trees.append(tree)
...
def evaluate_algorithm(df, algorithm, n_folds, ml_task, max_depth, sample_size, n_trees):
    ...
    for i, fold in enumerate(folds):
        ...
        test_df = fold
        predicted = algorithm(train_df, test_df, ml_task, max_depth, sample_size, n_trees)
...
scores = evaluate_algorithm(df, bagging, 5, "classification", 3, 0.50, 3)
